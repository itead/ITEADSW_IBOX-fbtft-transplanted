...
#else
                ret = itead_gpio_request_one(gpio->gpio, (uint32_t)flags);
#endif					
				if (ret < 0) {
...
int fbtft_backlight_update_status(struct backlight_device *bd)
{
	...
	if ((bd->props.power == FB_BLANK_UNBLANK) && (bd->props.fb_blank == FB_BLANK_UNBLANK))
		itead_gpio_set_value(par->gpio.led[0], polarity);
	else
		itead_gpio_set_value(par->gpio.led[0], !polarity);
...
void fbtft_register_backlight(struct fbtft_par *par)
{
	...
	if (!itead_gpio_get_value(par->gpio.led[0]))
...
void fbtft_reset(struct fbtft_par *par)
{
	...
	itead_gpio_set_value(par->gpio.reset, 0);
	udelay(20);
	itead_gpio_set_value(par->gpio.reset, 1);
	mdelay(120);
}
...
int fbtft_init_display(struct fbtft_par *par)
{
	...
	if (par->gpio.cs != -1)
		itead_gpio_set_value(par->gpio.cs, 0);  /* Activate chip */
