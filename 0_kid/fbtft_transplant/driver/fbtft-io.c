...
int fbtft_write_gpio16_wr(struct fbtft_par *par, void *buf, size_t len)
{
	...
	while (len) {
		...
		itead_gpio_set_value(par->gpio.wr, 0);
		...
		/* Pullup /WR */
		itead_gpio_set_value(par->gpio.wr, 1);
...
int fbtft_write_gpio16_wr_latched(struct fbtft_par *par, void *buf, size_t len)
{
	...
	while (len) {
		...
		/* Pulse 'latch' high */
		itead_gpio_set_value(par->gpio.latch, 1);
		itead_gpio_set_value(par->gpio.latch, 0);
...
#ifndef DO_NOT_OPTIMIZE_FBTFT_WRITE_GPIO
		if (data == prev_data) {
			itead_gpio_set_value(par->gpio.wr, 0); /* used as delay */
		} else {
			for (i = 0; i < 8; i++) {
				if ((data & 1) != (prev_data & 1))
					itead_gpio_set_value(par->gpio.db[i],
								(data & 1));
				data >>= 1;
				prev_data >>= 1;
			}
...
#else
		for (i = 0; i < 8; i++) {
			itead_gpio_set_value(par->gpio.db[i], (data & 1));
			data >>= 1;
		}
#endif
